...
| 12/30/2025 07:29 AM PST (via pst-timestamp) | **Task CRUD Operations** | Backend: Added PATCH /tasks/{id} for partial updates, POST /tasks/{id}/reopen for unchecking. Frontend: Created TaskCreateDialog with form (title, description, priority, due date, time estimate, tags). Wired TaskCard checkbox to useToggleTaskComplete mutation. Complete/reopen flow working end-to-end. All 4 pages (Tasks, Calendar, Notes, Goals) now display real API data. |
